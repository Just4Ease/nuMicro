...
type cache struct {
	...
	// registry cache
	sync.RWMutex
	cache   map[string][]*_registry.Service
...
func (c *cache) isValid(services []*_registry.Service, ttl time.Time) bool {
	// no services exist
	if len(services) == 0 {
		return false
	}
...
func (c *cache) get(service string) ([]*_registry.Service, error) {
	// read lock
	c.RLock()
	...
	get := func(service string, cached []*_registry.Service) ([]*_registry.Service, error) {
		// ask the registry
		...
		// cache results
		c.Lock()
		c.set(service, _registry.Copy(services))
		c.Unlock()
...
func (c *cache) set(service string, services []*_registry.Service) {
	c.cache[service] = services
	c.ttls[service] = time.Now().Add(c.opts.TTL)
}

func (c *cache) update(res *_registry.Result) {
...
func (c *cache) watch(w _registry.Watcher) error {
...
func (c *cache) GetService(service string) ([]*_registry.Service, error) {
	// get the service
	...
	if len(services) == 0 {
		return nil, _registry.ErrNotFound
	}
...
func New(r _registry.Registry, opts ...Option) Cache {
	...
	return &cache{
		...
		cache:    make(map[string][]*_registry.Service),
