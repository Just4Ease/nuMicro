...
import (
	...
	log "github.com/Just4Ease/nuMicro/utils/log"
)
...
type cache struct {
	...
	// registry cache
	sync.RWMutex
	cache   map[string][]*__registry.Service
...
func (c *cache) isValid(services []*__registry.Service, ttl time.Time) bool {
	// no services exist
	if len(services) == 0 {
		return false
	}
...
func (c *cache) get(service string) ([]*__registry.Service, error) {
	// read lock
	c.RLock()
	...
	get := func(service string, cached []*__registry.Service) ([]*__registry.Service, error) {
		// ask the registry
		...
		// cache results
		c.Lock()
		c.set(service, __registry.Copy(services))
		c.Unlock()
...
func (c *cache) set(service string, services []*__registry.Service) {
	c.cache[service] = services
	c.ttls[service] = time.Now().Add(c.opts.TTL)
}

func (c *cache) update(res *__registry.Result) {
	...
	var service *__registry.Service
	var index int
...
func (c *cache) watch(w __registry.Watcher) error {
...
func (c *cache) GetService(service string) ([]*__registry.Service, error) {
	// get the service
...
func New(r __registry.Registry, opts ...Option) Cache {
	...
	return &cache{
		...
		cache:    make(map[string][]*__registry.Service),
