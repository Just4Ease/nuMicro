...
import (
	...
	"github.com/Just4Ease/nuMicro/__registry"
...
func configure(e *etcdRegistry, opts ...__registry.Option) error {
...
func encode(s *__registry.Service) string {
	b, _ := json.Marshal(s)
	return string(b)
}

func decode(ds []byte) *__registry.Service {
	var s *__registry.Service
...
func (e *etcdRegistry) Init(opts ...__registry.Option) error {
	return configure(e, opts...)
}

func (e *etcdRegistry) Options() __registry.Options {
	return e.options
}

func (e *etcdRegistry) registerNode(s *__registry.Service, node *__registry.Node, opts ...__registry.RegisterOption) error {
	if len(s.Nodes) == 0 {
	...
	service := &__registry.Service{
		Name:      s.Name,
		Version:   s.Version,
		Metadata:  s.Metadata,
		Endpoints: s.Endpoints,
		Nodes:     []*__registry.Node{node},
	}

	var options __registry.RegisterOptions
	for _, o := range opts {
		o(&options)
	}
...
func (e *etcdRegistry) Deregister(s *__registry.Service) error {
	if len(s.Nodes) == 0 {
		return errors.New("Require at least one node")
	}
...
func (e *etcdRegistry) Register(s *__registry.Service, opts ...__registry.RegisterOption) error {
	if len(s.Nodes) == 0 {
		return errors.New("Require at least one node")
	}
...
func (e *etcdRegistry) GetService(name string) ([]*__registry.Service, error) {
	ctx, cancel := context.WithTimeout(context.Background(), e.options.Timeout)
	defer cancel()
	...
	if len(rsp.Kvs) == 0 {
		return nil, __registry.ErrNotFound
	}

	serviceMap := map[string]*__registry.Service{}

	for _, n := range rsp.Kvs {
		if sn := decode(n.Value); sn != nil {
			s, ok := serviceMap[sn.Version]
			if !ok {
				s = &__registry.Service{
					Name:      sn.Name,
					Version:   sn.Version,
	...
	services := make([]*__registry.Service, 0, len(serviceMap))
...
func (e *etcdRegistry) ListServices() ([]*__registry.Service, error) {
	versions := make(map[string]*__registry.Service)
	...
	services := make([]*__registry.Service, 0, len(versions))
...
func (e *etcdRegistry) Watch(opts ...__registry.WatchOption) (__registry.Watcher, error) {
