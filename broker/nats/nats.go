...
func (n *natsBroker) setAddresses(addrs []string) []string {
	//nolint:prealloc
	var cAddresses []string
	for _, addr := range addrs {
		...
		cAddresses = append(cAddresses, addr)
	}
	// if there's no address and we weren't told to
	// embed a local server then use the default url
	if len(cAddresses) == 0 && !n.local {
		cAddresses = []string{nats.DefaultURL}
	}
	return cAddresses
}
...
func (n *natsBroker) setOption(opts ...broker.Option) {
	...
	// only if Addresses, Secure or TLSConfig were not set through a broker.Option
	// we read them from nats.Option
	if len(n.opts.Addresses) == 0 && !n.local {
		n.opts.Addresses = n.nopts.Servers
	}
	...
	n.addrs = n.setAddresses(n.opts.Addresses)
